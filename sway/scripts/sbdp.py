...
cat_map={}
for config in docsList:
    cat_map[config.category]=[]

align=10
for config in docsList:
    # hotfix for bug in nwg
    ...
    string = "<b>{:<{}}</b> {}".format(config.keybinding, align, config.action)

    cat_map.setdefault(config.category,[]).append(string)
    #result = result + [{'category': config.category,
    #                    'action': config.action, 'keybinding': config.keybinding}]

for category, cat_list in cat_map.items():
    print(category)
